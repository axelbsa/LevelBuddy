...
class LB_PT_MainPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if ob is not None and len(bpy.context.selected_objects) > 0:
            ...
            col.prop(ob.lb_ObjectProperties, "brush_type", text="Brush Type")
            col.prop(ob.lb_ObjectProperties, "csg_operation", text="CSG Op")
            col.prop(ob.lb_ObjectProperties, "csg_order", text="CSG Order")
            col.prop(ob.lb_ObjectProperties, "brush_auto_texture", text="Auto Texture")

            if ob.lb_ObjectProperties.brush_auto_texture:
                col = layout.row(align=True)
                col.prop(ob.lb_ObjectProperties, "ceiling_texture_scale_offset")

                col = layout.row(align=True)
                col.prop(ob.lb_ObjectProperties, "wall_texture_scale_offset")

                col = layout.row(align=True)
                col.prop(ob.lb_ObjectProperties, "floor_texture_scale_offset")

                col = layout.row(align=True)
                col.prop(ob.lb_ObjectProperties, "ceiling_texture_rotation")

                col = layout.row(align=True)
                col.prop(ob.lb_ObjectProperties, "wall_texture_rotation")
            ...
            if ob.lb_ObjectProperties.brush_type == 'SECTOR' and ob.modifiers:
                ...
                col.prop(ob.lb_ObjectProperties, "ceiling_height")
                col.prop(ob.lb_ObjectProperties, "floor_height")
